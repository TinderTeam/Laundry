/**   
* @Title: ListItemTypeConst.java 
* @Package cn.fuego.eshoping.constant 
* @Description: TODO
* @author Aether
* @date 2014-12-21 下午8:27:45 
* @version V1.0   
*/ 
package cn.fuego.laundry.constant;

/** 
 * @ClassName: ListItemTypeConst 
 * @Description: TODO
 * @author Aether
 * @date 2014-12-21 下午8:27:45 
 *  
 */
public class ListItemTypeConst
{

	public static final int DEFAULT_CONTENT= 0 ; // 默认btn，无文字内容显示
	public static final int TEXT_CONTENT= 1 ; // btn带文字信息
	public static final int EDIT_TEXT= 2 ; // btn带文字信息
	public static final int BUTTON_ITEM= 3 ; // btn带文字信息
	public static final int IMG_CONTENT= 4 ; // btn带图片信息
	public static final int NULL_CONTENT= 5 ; //无内容，作为分割区域定义
	
}
